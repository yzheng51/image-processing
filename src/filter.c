...
void mosaic_transform_omp(pixel *pixels_o, pixel *pixels_i, int cols, int rows, int c) {
    ...
    begin = omp_get_wtime();
    // sum up all rgb values and stor in the average pixel
#pragma omp parallel for private(i, j) schedule(static, c)
    for (i = 0; i < rows; ++i) {
        for (j = 0; j < cols; ++j) {
            int offset = i * cols + j;
            int avg_offset = i / c * m_cols + j / c;
     ...
     */
    if (!mod_cols && !mod_rows) {
#pragma omp parallel for private(i, j) reduction(+ : average_r, average_g, average_b)
        for (i = 0; i < m_rows; ++i) {
            for (j = 0; j < m_cols; ++j) {
                int offset = i * m_cols + j;
                average_r += lpixels[offset].r;
...
#pragma omp parallel for private(i, j) reduction(+ : average_r, average_g, average_b)
        ...
        j = m_cols - 1;
#pragma omp parallel for private(i) reduction(+ : average_r, average_g, average_b)
        for (i = 0; i < m_rows; ++i) {
            int offset = i * m_cols + j;
            average_r += lpixels[offset].r;
            average_g += lpixels[offset].g;
...
#pragma omp parallel for private(i, j) reduction(+ : average_r, average_g, average_b)
        ...
        i = m_rows - 1;
#pragma omp parallel for private(j) reduction(+ : average_r, average_g, average_b)
        for (j = 0; j < m_cols; ++j) {
            int offset = i * m_cols + j;
            average_r += lpixels[offset].r;
            average_g += lpixels[offset].g;
